...
namespace SolverNameSpace
{
    ...
    bool NonlinearMPCData::IsValid(int numberOfDofs) const
    {
        ...
        const bool hasSparseHessians =
            static_cast<int>(hessians.size()) == nc;
        const bool hasLocalHessianEntries =
            static_cast<int>(hessianEntries.size()) == nc;
        if (numberOfDofs <= 0 || nc <= 0 || nc >= numberOfDofs
            || jacobian.rows() != nc || jacobian.cols() != numberOfDofs
            || (!hasSparseHessians && !hasLocalHessianEntries)
        ...
        for (int i = 0; i < nc; ++i)
        {
            if (slaveDofs[i] < 0 || slaveDofs[i] >= numberOfDofs
                ...
                return false;
            if (hasSparseHessians &&
                !((hessians[i].rows() == 0 && hessians[i].cols() == 0)
            ...
            if (hasLocalHessianEntries)
                for (const auto& entry : hessianEntries[i])
                    if (entry.row() < 0 || entry.row() >= numberOfDofs
                        || entry.col() < 0 || entry.col() >= numberOfDofs
        ...
        double rankTolerance)
    {
        ...
        if (tangent.cols() != ndof || rhs.size() != ndof
            || rankTolerance <= 0.0 || !rhs.allFinite()
            || !SparseAllFinite(tangent)
            || !constraints.IsValid(ndof))
            return false;
        ...
        const bool hasLocalHessianEntries =
            static_cast<int>(constraints.hessianEntries.size()) == nc;
        std::size_t hessianNonZeros = 0;
        for (int i = 0; i < nc; ++i)
            if (multiplier[i] != 0.0)
            {
                hessianNonZeros +=
                    static_cast<std::size_t>(constraints.hessians[i].nonZeros());
                if (hasLocalHessianEntries)
        ...
        if (hessianNonZeros > 0)
        {
            ...
            for (int i = 0; i < nc; ++i)
            {
                ...
                for (int outer = 0; outer < hessian.outerSize(); ++outer)
                    for (Eigen::SparseMatrix<double>::InnerIterator entry(
                             hessian, outer); entry; ++entry)
                        hessianTriplets.emplace_back(
                            entry.row(), entry.col(),
                            multiplier[i] * entry.value());
                if (hasLocalHessianEntries)
            ...
            lagrangianCorrection.setFromTriplets(
        ...
        if (diagonalSlaveBlock)
        {
            ...
            const auto accumulateTranspose =
                [&](const Eigen::VectorXd& full) -> Eigen::VectorXd
                {
            ...
            tangentTriplets.reserve(
                static_cast<std::size_t>(lagrangianTangent.nonZeros()) +
                static_cast<std::size_t>(nc) * 64);
            for (int outer = 0; outer < lagrangianTangent.outerSize(); ++outer)
            {
                for (Eigen::SparseMatrix<double>::InnerIterator entry(
                         lagrangianTangent, outer); entry; ++entry)
                {
                    ...
                    {
                        tangentTriplets.emplace_back(
                            rowMaster, columnMaster, entry.value());
                    }
                    else if (rowMaster >= 0)
                    {
                        for (const auto& [reducedColumn, coefficient] :
                             slaveTerms[slaveRow[column]])
                            tangentTriplets.emplace_back(
                                rowMaster, reducedColumn,
                                entry.value() * coefficient);
                    }
                    else if (columnMaster >= 0)
                    {
                        for (const auto& [reducedRow, coefficient] :
                             slaveTerms[slaveRow[row]])
                            tangentTriplets.emplace_back(
                                reducedRow, columnMaster,
                                coefficient * entry.value());
                    }
                    else
                    {
                        for (const auto& [reducedRow, rowCoefficient] :
                             slaveTerms[slaveRow[row]])
                            for (const auto& [reducedColumn, columnCoefficient] :
                                 slaveTerms[slaveRow[column]])
                                tangentTriplets.emplace_back(
                                    reducedRow, reducedColumn,
                                    rowCoefficient * entry.value() * columnCoefficient);
                    }
                }
            }
            reduction.tangent.resize(nm, nm);
            reduction.tangent.setFromTriplets(
                tangentTriplets.begin(), tangentTriplets.end());

            const Eigen::VectorXd reducedResidual =
                accumulateTranspose(structuralResidual);
        ...
        else
        {
            const Eigen::SparseMatrix<double> projected =
                lagrangianTangent * reduction.masterTransformation;
            reduction.tangent =
        ...
        return reduction.rhs.allFinite()
            && SparseAllFinite(reduction.tangent);
    }
}
