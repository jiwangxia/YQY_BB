...
struct RayleighDampingResult
{
    ...
    std::string ToText() const
    {
        ...
        output << std::setprecision(12) << "Rayleigh damping: C = alphaM*M + betaK*K\n"
...
inline RayleighCoefficients CalculateRayleighFromFrequencies(double frequency1Hz, double dampingRatio1,
                                                             double frequency2Hz, double dampingRatio2)
{
    if (!std::isfinite(frequency1Hz) || !std::isfinite(frequency2Hz) || frequency1Hz <= 0.0 || frequency2Hz <= 0.0)
        throw std::invalid_argument("Rayleigh reference frequencies must be finite and positive");

    if (!std::isfinite(dampingRatio1) || !std::isfinite(dampingRatio2) || dampingRatio1 < 0.0 || dampingRatio2 < 0.0)
    ...
    result.alphaM = 2.0 * omega1 * omega2 * (dampingRatio1 * omega2 - dampingRatio2 * omega1) / denominator;
    result.betaK = 2.0 * (dampingRatio2 * omega2 - dampingRatio1 * omega1) / denominator;

    const double coefficientScale = std::max(1.0, std::abs(result.alphaM) + std::abs(result.betaK));
    if (result.alphaM < -1.0e-12 * coefficientScale || result.betaK < -1.0e-12 * coefficientScale)
...
/**
     ...
     */
inline RayleighDampingResult SolveRayleighDamping(double frequency1Hz, double dampingRatio1, double frequency2Hz,
                                                  double dampingRatio2)
{
    constexpr double twoPi = 6.283185307179586476925286766559;
    RayleighDampingResult result;
    result.coefficients = CalculateRayleighFromFrequencies(frequency1Hz, dampingRatio1, frequency2Hz, dampingRatio2);
...
/** 相同目标阻尼比时的便捷重载。 */
inline RayleighDampingResult SolveRayleighDamping(double frequency1Hz, double frequency2Hz, double dampingRatio)
{
    return SolveRayleighDamping(frequency1Hz, dampingRatio, frequency2Hz, dampingRatio);
}
}
