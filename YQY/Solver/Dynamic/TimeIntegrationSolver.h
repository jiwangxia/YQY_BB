...
public:
    ...
    TimeIntegrationSolver(int ndof) : m_ndof(ndof) {}
    virtual ~TimeIntegrationSolver() = default;

    // 设置系统函数
    void setMassMatrix(MassMatrixFunc func) { m_getMass = func; }
    void setStiffnessMatrix(StiffnessMatrixFunc func) { m_getStiffness = func; }
    void setExternalForce(ForceFunc func) { m_getExternalForce = func; }
    void setInternalForce(ForceFunc func) { m_getInternalForce = func; }
    void setUpdateProperties(UpdatePropertiesFunc func) { m_updateProperties = func; }

    // 求解接口
    virtual bool solve(State& state, double dt, double t_end) = 0;
...
public:
    NewmarkSolver(int ndof, double beta = 0.25, double gamma = 0.5)
        : TimeIntegrationSolver(ndof), m_beta(beta), m_gamma(gamma) {}
...
private:
    ...
    bool solveSubstep(State& state_in, State& state_out,
                      double t, double dt, double c_factor);
};
...
public:
    ...
    int getRejectedSteps() const { return m_rejected_steps; }
    double getAverageStepSize() const { return m_avg_dt; }
...
private:
    ...
    void calculateParameters();
    bool solveSubstep(State& state_in, State& state_out,
                      double t, double dt, double c_factor, double alpha_factor);
    void calculateELDState(const State& state_n, const State& state_c1,
                          const State& state_c2, double dt);
    double estimateError(const State& state_order2, const State& state_order3,
 ...
 * // 1. 创建求解器
 * int ndof = 1000;
 * NewmarkSolver solver(ndof);
 *
 * // 2. 设置系统函数
 * solver.setMassMatrix([&](SparseMatrix<double>& M) {
 *     // 组装质量矩阵
 * });
 *
 * solver.setStiffnessMatrix([&](SparseMatrix<double>& K, const State& state) {
 *     // 组装刚度矩阵（可能依赖于当前状态）
 * });
 *
 * solver.setExternalForce([&](VectorXd& F, const State& state) {
 *     // 计算外力
 * });
 *
 * solver.setInternalForce([&](VectorXd& F, const State& state) {
 *     // 计算内力
 * });
 *
 * // 3. 初始化状态
 * TimeIntegrationSolver::State state;
 * state.u = VectorXd::Zero(ndof);
 * state.v = VectorXd::Zero(ndof);
 * state.a = VectorXd::Zero(ndof);
 * state.t = 0.0;
 *
 * // 4. 求解
 * double dt = 0.01;
 * double t_end = 10.0;
 * solver.solve(state, dt, t_end);
 */
