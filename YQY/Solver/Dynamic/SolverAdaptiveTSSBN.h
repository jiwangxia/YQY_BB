...
namespace SolverNameSpace
{
    ...
    public:
        ...
        const Params& GetParams() const { return parameters_; }
        int GetAcceptedStepCount() const { return acceptedStepCount_; }
        int GetRejectedStepCount() const { return rejectedStepCount_; }
        double GetAverageTimeStep() const { return averageTimeStep_; }
        double GetAverageStageNewtonIterations() const
        {
            return acceptedStepCount_ > 0
                ? static_cast<double>(totalStageNewtonIterations_)
                    / static_cast<double>(acceptedStepCount_)
                : 0.0;
        }
    ...
    private:
        ...
        double previousAcceptedError_ = 1.0;
        ...
        ImplicitStageDefinition CreateFirstStageDefinition(
            int dofCount) const;
        ImplicitStageDefinition CreateSecondStageDefinition(
            double timeStep,
            const StageState& firstStage) const;

        bool SolveImplicitStage(
        ...
        double EstimateNormalizedError(
        ...
        double ProposeAcceptedTimeStep(
            double currentTimeStep,
            double normalizedError,
            int totalStageIterations);

        double ProposeRejectedTimeStep(
            double currentTimeStep,
