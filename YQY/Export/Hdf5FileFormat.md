...
| 写法 | 说明 |
| --- | --- |
| `*OUTPUT, H5, 1` | 开启 H5 输出。默认输出到 `Export/ExportFile/<输入模型名>.h5`。 |
| `*OUTPUT, H5, 1, result.h5` | 开启 H5 输出，并指定 H5 文件名。相对路径会输出到 `Export/ExportFile` 下。 |
| `*OUTPUT, H5, 0` | 关闭 H5 输出。 |

当前保存策略如下：

| 分析类型 | `H5=0` | `H5=1` |
| --- | --- | --- |
| 静力分析 | 默认保存内存结果。 | 计算完成后统一写入 H5。 |
| 动力分析 | 默认不保存完整历史结果，只保留当前计算状态。 | 计算过程中每个时间步流式写入 H5。 |
