...
#### 3.8.1 `/YQY/INPUT/ANALYSIS_STEP/METADATA`

分析步的名称、计算区域范围和初始平衡状态依赖。动力步勾选“使用静力步作为初始状态”后，`INITIAL_STATIC_STEP_ID` 保存其引用的静力步；求解时先独立计算该静力步，再复制其末态作为动力步的 `t = 0` 状态。多个动力步可以引用同一静力步，各动力步均为彼此隔离的分支：只继承该静力步已经生效的持续荷载、约束和 MPC，再叠加当前动力步自己的定义，不会因为分析步编号较小而吸收其他动力步的数据。单独计算某个动力步时，前置静力步仅作为内部初态计算，不写入该动力算例的结果帧；动力步的 `t = 0` 帧即为复制后的静力平衡状态。

| 字段名 | 类型 | 说明 |
| --- | --- | --- |
| `STEP_ID` | int | 当前分析步编号。 |
| `REGION_SCOPE` | int | 当前分析步的计算区域选择方式。 |
| `INITIAL_STATIC_STEP_ID` | int | 动力步绑定的前置静力步编号，必须小于当前动力步编号；`0` 表示独立计算、不继承静力平衡状态。 |
| `NAME` | string | 分析步名称，UTF-8。 |
| `DOMAIN_ID` | int | 模型域编号，当前固定为 `1`。 |
