...
public:
    ElementResultExportDialog(const QString& resultFile, const QSet<int>& availableElements, QWidget* parent)
        : QDialog(parent), m_availableElements(availableElements)
    {
        ...
        auto* selectionHint = new QLabel(QStringLiteral("支持逗号、空格和连续范围；导出前会检查单元是否存在。"), selectionCard);
        ...
        connect(m_elementEdit, &QLineEdit::textChanged, this, [this]() { updateValidation(); });
        connect(m_fileEdit, &QLineEdit::textChanged, this, [this]() { updateValidation(); });
        connect(browseButton, &QPushButton::clicked, this, [this]() {
            const QString file = QFileDialog::getSaveFileName(this, QStringLiteral("保存单元结果"), m_fileEdit->text(),
                QStringLiteral("BDF 结果文件 (*.bdf);;文本文件 (*.txt)"), nullptr, QFileDialog::DontUseNativeDialog);
            if (!file.isEmpty()) m_fileEdit->setText(file);
        });
        connect(buttons, &QDialogButtonBox::rejected, this, &QDialog::reject);
        connect(buttons, &QDialogButtonBox::accepted, this, [this]() { if (updateValidation()) accept(); });
        resize(700, 500);
        DialogSizing::lockHeightToContents(this);
        updateValidation();
    }

    std::vector<int> elementIds() const { QString error; return parseElementIds(error); }
    std::vector<EnumKeyword::ElementResultType> resultTypes() const
    {
        std::vector<EnumKeyword::ElementResultType> result;
        for (const auto& field : m_fields) if (field.box->isChecked()) result.push_back(field.type);
        return result;
    }
    QString outputFile() const { return QFileInfo(m_fileEdit->text().trimmed()).absoluteFilePath(); }

private:
    struct Field { QCheckBox* box; EnumKeyword::ElementResultType type; };
    ...
    void addFieldGroup(QVBoxLayout* layout, const QString& caption, std::initializer_list<FieldDefinition> definitions)
    {
        ...
        for (const auto& definition : definitions) {
            auto* box = new QCheckBox(definition.label, group); box->setChecked(definition.checked); row->addWidget(box);
            m_fields.push_back({box, definition.type}); connect(box, &QCheckBox::toggled, this, [this]() { updateValidation(); });
        }
        row->addStretch(); layout->addWidget(group);
    }
    std::vector<int> parseElementIds(QString& error) const
    {
        QSet<int> ids;
        const QStringList tokens = m_elementEdit->text().split(QRegularExpression(QStringLiteral("[,，;；\\s]+")), Qt::SkipEmptyParts);
        const QRegularExpression rangePattern(QStringLiteral("^(\\d+)\\s*[-~～]\\s*(\\d+)$"));
        for (const QString& token : tokens) {
            const auto match = rangePattern.match(token); bool ok = false; int first = token.toInt(&ok); int last = first;
            if (match.hasMatch()) { first = match.captured(1).toInt(); last = match.captured(2).toInt(); ok = true; }
            if (!ok || first <= 0 || last < first || last - first > 100000) { error = QStringLiteral("无法识别单元编号“%1”").arg(token); return {}; }
            for (int id = first; id <= last; ++id) ids.insert(id);
        }
        QList<int> sorted = ids.values(); std::sort(sorted.begin(), sorted.end());
        for (int id : sorted) if (!m_availableElements.contains(id)) { error = QStringLiteral("模型中不存在单元 %1").arg(id); return {}; }
        return std::vector<int>(sorted.begin(), sorted.end());
    }
    bool updateValidation()
    {
        QString error; const auto ids = parseElementIds(error);
        if (error.isEmpty() && ids.empty()) error = QStringLiteral("请输入至少一个单元编号");
        if (error.isEmpty() && resultTypes().empty()) error = QStringLiteral("请至少勾选一个结果分量");
        if (error.isEmpty() && m_fileEdit->text().trimmed().isEmpty()) error = QStringLiteral("请选择输出文件");
        if (error.isEmpty() && !QDir(QFileInfo(m_fileEdit->text().trimmed()).absolutePath()).exists()) error = QStringLiteral("输出文件夹不存在");
        m_errorLabel->setText(error); m_exportButton->setEnabled(error.isEmpty()); return error.isEmpty();
    }
    QSet<int> m_availableElements;
    QLineEdit* m_elementEdit = nullptr; QLineEdit* m_fileEdit = nullptr; QLabel* m_errorLabel = nullptr; QPushButton* m_exportButton = nullptr;
