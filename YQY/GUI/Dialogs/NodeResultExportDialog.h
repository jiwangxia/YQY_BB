...
{
public:
    NodeResultExportDialog(const QString& resultFile, const QSet<int>& availableNodes,
        int selectedNodeId, QWidget* parent)
        : QDialog(parent)
        , m_availableNodes(availableNodes)
    {
        ...
        for (auto* group : { form.displacementGroup, form.rotationGroup, form.velocityGroup,
            form.accelerationGroup, form.coordinateGroup, form.reactionGroup })
        ...
        if (selectedNodeId > 0 && availableNodes.contains(selectedNodeId))
        {
            ...
            QObject::connect(currentButton, &QPushButton::clicked, this,
                [this, selectedNodeId]() { m_nodeEdit->setText(QString::number(selectedNodeId)); });
            m_nodeEdit->setText(QString::number(selectedNodeId));
        }
        const std::initializer_list<FieldCheckBox> fieldDefinitions = {
            {form.u1,EnumKeyword::NodeResultType::U1},{form.u2,EnumKeyword::NodeResultType::U2},{form.u3,EnumKeyword::NodeResultType::U3},{form.um,EnumKeyword::NodeResultType::MagnitudeU},
            {form.ur1,EnumKeyword::NodeResultType::UR1},{form.ur2,EnumKeyword::NodeResultType::UR2},{form.ur3,EnumKeyword::NodeResultType::UR3},
            {form.v1,EnumKeyword::NodeResultType::V1},{form.v2,EnumKeyword::NodeResultType::V2},{form.v3,EnumKeyword::NodeResultType::V3},
            {form.a1,EnumKeyword::NodeResultType::A1},{form.a2,EnumKeyword::NodeResultType::A2},{form.a3,EnumKeyword::NodeResultType::A3},
            {form.cx,EnumKeyword::NodeResultType::CX},{form.cy,EnumKeyword::NodeResultType::CY},{form.cz,EnumKeyword::NodeResultType::CZ},
            {form.r1,EnumKeyword::NodeResultType::R1},{form.r2,EnumKeyword::NodeResultType::R2},{form.r3,EnumKeyword::NodeResultType::R3} };
        for (const auto& field : fieldDefinitions) { m_fields.push_back(field); QObject::connect(field.box,&QCheckBox::toggled,this,[this](){updateValidation();}); }

        m_fileEdit = form.fileEdit;
        const QFileInfo resultInfo(resultFile);
        m_fileEdit->setText(QDir(nodeResultExportDirectory()).absoluteFilePath(
            resultInfo.completeBaseName() + QStringLiteral("_节点时程.bdf")));
        ...
        QObject::connect(browseButton, &QPushButton::clicked, this, [this]() {
            const QString selected = QFileDialog::getSaveFileName(this,
                QStringLiteral("保存节点结果"), m_fileEdit->text(),
...
private:
    ...
    std::vector<int> parseNodeIds(QString& error) const
    {
        QSet<int> ids;
        const QStringList tokens = m_nodeEdit->text().split(
        ...
        if (!missing.isEmpty())
        {
            ...
            error = QStringLiteral("模型中不存在节点：%1%2").arg(values.join(QStringLiteral(", ")),
                missing.size() > 6 ? QStringLiteral(" …") : QString());
            return {};
        }
    ...
    bool updateValidation()
    {
        ...
        if (error.isEmpty()
            && !QDir(QFileInfo(m_fileEdit->text().trimmed()).absolutePath()).exists())
