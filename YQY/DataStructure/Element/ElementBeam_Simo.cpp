...
void ElementBeam_Simo::GetSectionData(double& area, double& Iy, double& Iz,
    double& J, double& young, double& shear, double& density) const
{
    ...
    if (area <= 0.0 || young <= 0.0 || density < 0.0
        || 1.0 + material->m_Poisson <= 0.0)
...
void ElementBeam_Simo::Get_L0()
{
    ...
    if (e2.norm() <= 1.0e-10)
    {
        const Eigen::Vector3d auxiliary = std::abs(e1.x()) < 0.9
...
ElementBeam_Simo::GaussState ElementBeam_Simo::EvaluateGaussState(
    double coordinate) const
{
    ...
    const Eigen::Vector3d x0(node0->m_X + node0->m_Displacement[0],
        node0->m_Y + node0->m_Displacement[1],
        node0->m_Z + node0->m_Displacement[2]);
    const Eigen::Vector3d x1(node1->m_X + node1->m_Displacement[0],
        node1->m_Y + node1->m_Displacement[1],
    ...
    state.strain = state.chord / L0
        - state.rotation * Eigen::Vector3d::UnitX();
    return state;
}
...
void ElementBeam_Simo::Get_ke(Eigen::MatrixXd& ke)
{
    ...
    Eigen::Matrix<double, 6, 6> constitutive =
    ...
    const Eigen::Vector3d force = state.rotation * forceConstitutive
        * state.rotation.transpose() * state.strain;
    const Eigen::Vector3d moment = state.rotation * momentConstitutive
        * state.rotation.transpose() * state.curvature;
    ...
    const Eigen::Matrix<double, 6, 6> spatialConstitutive =
        rotationBlock * constitutive * rotationBlock.transpose();
    Eigen::Matrix<double, 12, 12> geometric =
    ...
    for (int i = 0; i < 2; ++i)
    {
        for (int j = 0; j < 2; ++j)
        {
            ...
            block.block<3, 3>(3, 3) = Skew(-moment) * dN[i] * N[j]
                + (force * tangent.transpose() - force.dot(tangent) * identity)
                    * N[i] * N[j];
...
void ElementBeam_Simo::Get_me_Consistent(Eigen::MatrixXd& me)
{
    ...
    for (double point : points)
    {
        ...
        const Eigen::Matrix3d spatialInertia =
            state.rotation * materialInertia * state.rotation.transpose();
        for (int i = 0; i < 2; ++i)
        {
            for (int j = 0; j < 2; ++j)
            {
                const double weight = 0.5 * L0 * N[i] * N[j];
                me.block<3, 3>(6 * i, 6 * j) +=
                    weight * density * area * Eigen::Matrix3d::Identity();
                me.block<3, 3>(6 * i + 3, 6 * j + 3) +=
                    weight * spatialInertia;
            }
        }
    }
    m_me = me;
...
void ElementBeam_Simo::Assemble(const std::vector<double>& damping,
    Eigen::MatrixXd& ce)
{
    if (damping.size() < 4)
