...
double ZeroCrossing(double firstAngle, double secondAngle,
    double firstValue, double secondValue)
{
    const double difference = secondValue - firstValue;
    if (std::abs(difference) <= kTolerance)
        return firstAngle;
    return firstAngle + (secondAngle - firstAngle)
        * (-firstValue / difference);
}
}

std::vector<GallopingInstabilityInterval>
GallopingStabilityAnalyzer::FindNegativeDampingIntervals(
    const std::vector<BladeModel>& models, double angleStepDegrees)
{
    ...
    for (int profileId = 0; profileId < static_cast<int>(models.size()); ++profileId)
    {
        ...
        {
            const double liftSlope = (model.lift[index + 1] - model.lift[index])
                / angleStepRadians;
            ...
            const double negativeStart = firstH < 0.0
                ? firstAngle : ZeroCrossing(firstAngle, secondAngle, firstH, secondH);
            const double negativeEnd = secondH < 0.0
                ? secondAngle : ZeroCrossing(firstAngle, secondAngle, firstH, secondH);
            ...
            if (!intervals.empty())
            {
                ...
                if (previous.profileId == profileId
                    && std::abs(previous.endAngleDegrees - negativeStart) <= kTolerance)
                {
                    previous.endAngleDegrees = negativeEnd;
                    previous.minimumDenHartog = std::min(
                        previous.minimumDenHartog, minimumH);
                    continue;
                }
            }
