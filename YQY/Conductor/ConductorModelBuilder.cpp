...
namespace Conductor
{
static bool FindCableAxisAtNode(const StructureData& structure, int nodeId, _OUT Vector3d& axis)
{
    Vector3d sum = Vector3d::Zero();
    for (const auto& [elementId, element] : structure.m_Elements)
    {
        Q_UNUSED(elementId);
        if (!std::dynamic_pointer_cast<ElementCable>(element) || element->m_pNode.size() != 2)
            continue;
        const auto first = element->m_pNode[0].lock();
        const auto second = element->m_pNode[1].lock();
        if (!first || !second || (first->m_Id != nodeId && second->m_Id != nodeId))
            continue;
        const Vector3d chord(second->m_X - first->m_X, second->m_Y - first->m_Y, second->m_Z - first->m_Z);
        if (chord.norm() > 1.0e-12)
            sum += Utility::CR::CanonicalAxis(chord);
    }
    if (sum.norm() <= 1.0e-12)
        return false;
    axis = Utility::CR::CanonicalAxis(sum);
    return true;
}
...
bool ConductorModelBuilder::BuildLine(const LineBuildConfig& config, _OUT LineBuildResult& result,
                                      _OUT std::string& error)
{
    ...
    if (config.conductor.nBundle != 1 && config.conductor.nBundle != 2 && config.conductor.nBundle != 4 &&
        config.conductor.nBundle != 6 && config.conductor.nBundle != 8)
    {
    ...
    BundleResult raw = Generator::CreateBundle(config.start.data(), config.end.data(), leftCutLength, rightCutLength,
...
bool ConductorModelBuilder::BuildSpanConductor(const SpanConductorBuildConfig& config, _OUT LineBuildResult& result,
                                               _OUT std::string& error)
{
    ...
    auto rollbackLine = [&]()
    {
        ...
        {
            if (spacer.rigidBodyInertiaId > 0)
                m_structure->m_RigidBodyInertias.erase(spacer.rigidBodyInertiaId);
...
bool ConductorModelBuilder::BuildMultiSpanConductor(const MultiSpanConductorBuildConfig& config,
                                                    _OUT LineBuildResult& result, _OUT std::string& error)
{
    result = LineBuildResult{};
    if (!m_structure)
    {
    ...
    if (lineConfig.conductor.nBundle != 1 && lineConfig.conductor.nBundle != 2 && lineConfig.conductor.nBundle != 4 &&
        lineConfig.conductor.nBundle != 6 && lineConfig.conductor.nBundle != 8)
    {
        error = "导线分裂数仅支持 1、2、4、6、8";
        return false;
    }
    ...
    if (lineConfig.conductor.segments <= 0 || lineConfig.conductor.initialStress <= 0.0 ||
    ...
    auto suspensionProperty = config.suspensionProperty ? config.suspensionProperty : lineConfig.property;
    if (!ValidateProperty(suspensionProperty, "悬垂串", error))
        return false;
    auto fittingProperty = lineConfig.endFittingProperty ? lineConfig.endFittingProperty : lineConfig.property;
    ...
    std::set<int> originalInertiaIds;
    ...
    for (const auto& [id, inertia] : m_structure->m_RigidBodyInertias)
        originalInertiaIds.insert(id);
    auto rollback = [&]()
    {
        ...
        for (auto it = m_structure->m_RigidBodyInertias.begin(); it != m_structure->m_RigidBodyInertias.end();)
        {
            if (originalInertiaIds.find(it->first) == originalInertiaIds.end())
                it = m_structure->m_RigidBodyInertias.erase(it);
            else
                ++it;
        }
    ...
    for (std::size_t spanIndex = 0; spanIndex < spanCount; ++spanIndex)
    {
        ...
        const double transitionLength =
            std::min(std::max(0.0, lineConfig.bundleEndTransitionLength), horizontalLength * 0.1);
        BundleResult raw = Generator::CreateBundle(start.data(), end.data(), transitionLength, transitionLength,
    ...
    const bool directWireSupports = wireCount > 1 && lineConfig.endTopology == BundleEndTopology::DirectWireSupports;
    auto createTensionEnds = [&]() -> bool
    {
        ...
        for (int wireId = 0; wireId < wireCount; ++wireId)
        {
            ...
            if (leftNodes.size() < 2 || rightNodes.size() < 2)
                return false;
            leftSums[group] +=
                Vector3d((leftNodes[0].x + leftNodes[1].x) * 0.5, (leftNodes[0].y + leftNodes[1].y) * 0.5,
                         (leftNodes[0].z + leftNodes[1].z) * 0.5);
            const std::size_t last = rightNodes.size() - 1;
            rightSums[group] += Vector3d((rightNodes[last - 1].x + rightNodes[last].x) * 0.5,
        ...
        for (int group = 0; group < groupCount; ++group)
        {
            ...
            int leftGroupId = -1;
            int rightGroupId = -1;
            if (!addNode(leftCenters[group], leftGroupId) || !addNode(rightCenters[group], rightGroupId))
                return false;
        ...
        if (!dualSupport)
        {
            if (!addNode(result.start, result.leftSupportNodeId) || !addNode(result.end, result.rightSupportNodeId))
                return false;
            result.leftTensionEnd.supportNodeId = result.leftSupportNodeId;
            result.rightTensionEnd.supportNodeId = result.rightSupportNodeId;
        ...
        const double spacing =
            lineConfig.dualSupportSpacing > 1.0e-7 ? lineConfig.dualSupportSpacing : lineConfig.conductor.spacing;
        auto addDual = [&](const Vector3d& base, const std::vector<Vector3d>& centers, TensionEndModel& endModel,
                           int& primaryId) -> bool
        {
    ...
    auto stationDirection = [&](std::size_t stationIndex) -> Vector2d
    {
        ...
        {
            Vector2d next = (config.stationCenters[stationIndex + 1] - config.stationCenters[stationIndex]).head<2>();
            if (next.norm() > 1.0e-7)
                direction += next.normalized();
        }
        if (direction.norm() <= 1.0e-7 && stationIndex + 1 < config.stationCenters.size())
            direction = (config.stationCenters[stationIndex + 1] - config.stationCenters[stationIndex]).head<2>();
    ...
    auto bundleOffset = [&](int wireId, const Vector2d& direction) -> Vector3d
    {
        ...
        else if (wireCount == 4)
        {
            ...
            radius = lineConfig.conductor.spacing / (2.0 * std::sin(std::numbers::pi / 4.0));
        }
        else if (wireCount == 6)
        {
            startAngle = -std::numbers::pi / 3.0;
            radius = lineConfig.conductor.spacing / (2.0 * std::sin(std::numbers::pi / 6.0));
        }
        else if (wireCount == 8)
        {
            startAngle = -3.0 * std::numbers::pi / 8.0;
            radius = lineConfig.conductor.spacing / (2.0 * std::sin(std::numbers::pi / 8.0));
        }
        const double angle = startAngle + 2.0 * std::numbers::pi * wireId / wireCount;
        const Vector2d right(direction.y(), -direction.x());
        return Vector3d(right.x() * std::cos(angle) * radius, right.y() * std::cos(angle) * radius,
    ...
    for (std::size_t stationIndex = 1; stationIndex + 1 < config.stationCenters.size(); ++stationIndex)
    {
        ...
        if (wireCount > 1)
        {
            rise = lineConfig.conductor.spacing * 0.5 * (1.0 + 1.0 / std::tan(std::numbers::pi / wireCount));
        }
        ...
        if (!addNode(junctionPosition + Vector3d(0.0, 0.0, config.suspensionStringLength), suspension.supportNodeId))
        {
            error = "创建中间悬垂约束挂点失败";
            rollback();
            return false;
        ...
        {
            int elementId = -1;
            if (!AddElement(suspension.wireNodeIds[wireId], suspension.junctionNodeId,
                            // H 型悬垂顶部两根联板按杆单元处理，使 J-P 悬垂串和
                            // 上部挂点仅引入平动自由度，避免转角自由度导致奇异。
                            EnumKeyword::ElementType::T3D2, lineConfig.property, lineConfig.conductor.initialStress,
                            elementId, error, ElementRole::SuspensionHardware))
            {
        ...
        if (!AddElement(suspension.junctionNodeId, suspension.supportNodeId, config.suspensionElementType,
                        suspensionProperty, lineConfig.conductor.initialStress, suspension.stringElementId, error,
    ...
    for (std::size_t spanIndex = 0; spanIndex < spanCount; ++spanIndex)
    {
        ...
        for (int wireId = 0; wireId < wireCount; ++wireId)
        {
            ...
            {
                int nodeId = -1;
                if (!addNode(Vector3d(rawNodes[nodeIndex].x, rawNodes[nodeIndex].y, rawNodes[nodeIndex].z), nodeId))
                {
                    error = "添加多档导线内部节点失败";
                    rollback();
                    return false;
            ...
            globalNodes.insert(globalNodes.end(), std::next(spanSub.nodeIds.begin()), spanSub.nodeIds.end());
        }
        ...
        for (const auto& [wireId, rawElements] : raw.wiresElement)
        {
            ...
            for (const auto& rawElement : rawElements)
            {
                ...
                if (iIndex < 0 || jIndex < 0 || iIndex >= static_cast<int>(rawNodes.size()) ||
                ...
                int elementId = -1;
                if (!AddElement(rawNodes[iIndex].id, rawNodes[jIndex].id, lineConfig.elementType, lineConfig.property,
    ...
    {
        auto& spanResult = spanResults[spanIndex];
        if (!config.span.innerSpacers.empty() && !BuildInnerSpacers(spanResult, config.span.innerSpacers, error))
        {
            rollback();
            return false;
        }
        ...
        {
            std::vector<InnerSpacerConfig> autoSpacers;
            if (!CalculateInnerSpacerConfigs(spanResult, config.span.innerSpacerLayout, autoSpacers, error) ||
        ...
        result.innerSpacers.insert(result.innerSpacers.end(), spanResult.innerSpacers.begin(),
                                   spanResult.innerSpacers.end());
    }

    if (!CreateSubConductorSets(lineConfig, result, error) || !RenumberLineModel(result, error))
    {
        rollback();
        return false;
    }
...
int ConductorModelBuilder::NextRigidBodyInertiaId() const
{
    if (!m_structure || m_structure->m_RigidBodyInertias.empty())
        return 1;
    return m_structure->m_RigidBodyInertias.rbegin()->first + 1;
}

bool ConductorModelBuilder::ValidateProperty(std::shared_ptr<Property> property, const std::string& objectName,
...
bool ConductorModelBuilder::AddRigidSpacerInertia(int centerNodeId, const std::vector<Vector3d>& wirePositions,
                                                  const std::shared_ptr<Property>& property, _OUT int& inertiaId,
                                                  _OUT std::string& error)
{
    inertiaId = -1;
    const auto centerNode = m_structure ? m_structure->FindNode(centerNodeId) : nullptr;
    ...
    if (!centerNode || centerNode->m_DOF.size() < 6 || !material || !section || material->m_Density <= 0.0 ||
        section->m_Area <= 0.0)
    {
        error = "刚性中心间隔棒无法计算有效质量属性";
        return false;
    }

    const Vector3d center(centerNode->m_X, centerNode->m_Y, centerNode->m_Z);
    const double lineMass = material->m_Density * section->m_Area;
    double mass = 0.0;
    double maximumArmLength = 0.0;
    Vector3d firstMassMoment = Vector3d::Zero();
    Matrix3d rotaryInertia = Matrix3d::Zero();
    for (const Vector3d& wirePosition : wirePositions)
    {
        const Vector3d offset = wirePosition - center;
        const double length = offset.norm();
        if (length <= 1.0e-10)
        {
            error = "刚性中心间隔棒存在长度为零的等效刚臂";
            return false;
        }
        const double armMass = lineMass * length;
        mass += armMass;
        maximumArmLength = std::max(maximumArmLength, length);
        firstMassMoment += 0.5 * armMass * offset;
        // 每根中心刚臂按均匀细杆等效：m=ρAL，关于中心端点的转动惯量系数为 m/3。
        rotaryInertia += (armMass / 3.0) *
                          (offset.squaredNorm() * Matrix3d::Identity() - offset * offset.transpose());
    }
    if (firstMassMoment.norm() > 1.0e-8 * mass * maximumArmLength)
    {
        error = "刚性中心间隔棒的参考节点与等效质心不重合";
        return false;
    }

    auto inertia = std::make_shared<RigidBodyInertia>();
    inertia->m_Id = NextRigidBodyInertiaId();
    inertia->m_pNode = centerNode;
    inertia->m_Mass = mass;
    inertia->m_RotaryInertia = 0.5 * (rotaryInertia + rotaryInertia.transpose());
    if (!inertia->IsValid() || !m_structure->m_RigidBodyInertias.emplace(inertia->m_Id, inertia).second)
    {
        error = "添加刚性中心间隔棒集中惯性失败";
        return false;
    }
    inertiaId = inertia->m_Id;
    return true;
}

std::shared_ptr<ElementBase> ConductorModelBuilder::CreateLineElement(EnumKeyword::ElementType elementType,
                                                                      _OUT std::string& error) const
{
    switch (elementType)
    {
...
int ConductorModelBuilder::FindNearestNodeOnSubConductor(const SubConductorModel& sub, const Vector3d& leftBase,
                                                         const Vector2d& direction, double targetDistance,
...
bool ConductorModelBuilder::AddElement(int iNodeId, int jNodeId, EnumKeyword::ElementType elementType,
                                       std::shared_ptr<Property> property, double initStress, _OUT int& elementId,
                                       _OUT std::string& error, ElementRole role, int wireId, int aeroProfileId,
                                       int aeroBundleCount)
{
...
bool ConductorModelBuilder::BuildSuspensionSpacer(_OUT SuspensionPointModel& suspension,
                                                  const InnerSpacerConfig& config, _OUT std::string& error)
{
    if (suspension.wireNodeIds.size() < 2)
        return true;
    const bool rigidCenterMpc = config.style == InnerSpacerStyle::RigidCenterMPC;
    ...
    if (!ValidateProperty(config.property, "悬垂端相内间隔棒", error))
        return false;
    if (!rigidCenterMpc && config.elementType == EnumKeyword::ElementType::UNKNOWN)
    {
    ...
    int createdInertiaId = -1;
    auto rollback = [&]()
    {
        for (int mpcId : createdMpcIds)
            m_structure->m_MPCConstraints.erase(mpcId);
        if (createdInertiaId > 0)
            m_structure->m_RigidBodyInertias.erase(createdInertiaId);
        ...
        suspension.spacerRigidBodyInertiaId = -1;
    };
    auto addNode = [&](const Vector3d& position, int& nodeId) -> bool
    {
        nodeId = NextNodeId();
    ...
    if (rigidCenterMpc)
    {
        ...
        const auto master = m_structure->FindNode(suspension.spacerCenterNodeId);
        master->SetNumDOFs(6);
        for (std::size_t wireId = 0; wireId < suspension.wireNodeIds.size(); ++wireId)
        {
            const auto slave = m_structure->FindNode(suspension.wireNodeIds[wireId]);
            const int mpcId = NextMPCId();
            auto mpc = std::make_shared<RigidOffsetMPCConstraint>();
            mpc->m_Id = mpcId;
            mpc->m_Name = QStringLiteral("悬垂端刚性中心间隔棒_%1").arg(mpcId);
            ...
            Vector3d cableAxis;
            if (!FindCableAxisAtNode(*m_structure, slave->m_Id, cableAxis))
            {
                error = "无法确定悬垂端索节点的局部轴向";
                rollback();
                return false;
            }
            const int twistMpcId = NextMPCId();
            auto twistMpc = std::make_shared<AxialTwistTieMPCConstraint>();
            twistMpc->m_Id = twistMpcId;
            twistMpc->m_Name = QStringLiteral("悬垂端间隔棒轴向扭转_%1").arg(twistMpcId);
            twistMpc->m_pMasterNode = master;
            twistMpc->m_pSlaveNode = slave;
            twistMpc->m_Axis = cableAxis;
            twistMpc->m_SlaveDirections = {3};
            if (!m_structure->m_MPCConstraints.emplace(twistMpcId, twistMpc).second)
            {
                error = "添加悬垂端间隔棒轴向扭转 MPC 失败";
                rollback();
                return false;
            }
            createdMpcIds.push_back(twistMpcId);
            suspension.spacerMpcIds.push_back(twistMpcId);
        }
        if (!AddRigidSpacerInertia(suspension.spacerCenterNodeId, wirePositions, config.property, createdInertiaId,
                                   error))
        {
            rollback();
            return false;
        }
        suspension.spacerRigidBodyInertiaId = createdInertiaId;
        return true;
    }

    const std::vector<int>& spacerWireNodeIds = suspension.wireNodeIds;
    ...
    for (int wireId = 0; wireId < outerEdgeCount; ++wireId)
    {
        ...
        if (!addSpacerElement(spacerWireNodeIds[wireId], spacerWireNodeIds[nextWireId]))
        {
            rollback();
            return false;
        }
    ...
    if (config.style == InnerSpacerStyle::CenterBraced)
    {
        ...
        for (int wireId = 0; wireId < wireCount; ++wireId)
        {
            if (!addSpacerElement(spacerWireNodeIds[wireId], suspension.spacerCenterNodeId))
            {
                rollback();
                return false;
            }
    ...
    for (int wireId = 0; wireId < wireCount; ++wireId)
    {
        ...
        if (!addSpacerElement(suspension.spacerInnerNodeIds[wireId], suspension.spacerInnerNodeIds[nextWireId]) ||
            !addSpacerElement(spacerWireNodeIds[wireId], suspension.spacerInnerNodeIds[wireId]))
        {
            rollback();
            return false;
        }
...
bool ConductorModelBuilder::BuildInnerSpacer(LineBuildResult& line, const InnerSpacerConfig& config,
                                             _OUT InnerSpacerModel& spacer, _OUT std::string& error)
{
    ...
    std::vector<Vector3d> wirePositions;
    wirePositions.reserve(line.subConductors.size());
    std::vector<Vector3d> wireAxes;
    wireAxes.reserve(line.subConductors.size());

    Vector3d center = Vector3d::Zero();
    for (const auto& pair : line.subConductors)
    {
        const bool excludeEndpoints = spacer.position > 1e-7 && spacer.position < line.spanLength - 1e-7;
        int nodeId =
            FindNearestNodeOnSubConductor(pair.second, line.start, direction, spacer.position, excludeEndpoints);
        auto node = m_structure->FindNode(nodeId);
        if (!node)
        {
            error = "相内间隔棒找不到子导线节点";
        ...
        wirePositions.emplace_back(node->m_X, node->m_Y, node->m_Z);
        Vector3d cableAxis;
        if (!FindCableAxisAtNode(*m_structure, nodeId, cableAxis))
        {
            error = "无法确定相内间隔棒索节点的局部轴向";
            return false;
        }
        wireAxes.push_back(cableAxis);
    ...
    int centerNodeId = -1;
    if (wireNodeIds.size() > 2 && (config.style == InnerSpacerStyle::CenterBraced || rigidCenterMpc))
    {
    ...
    if (rigidCenterMpc)
    {
        ...
        for (std::size_t wireId = 0; wireId < wireNodeIds.size(); ++wireId)
        {
            ...
            auto mpc = std::make_shared<RigidOffsetMPCConstraint>();
            mpc->m_Id = mpcId;
            mpc->m_Name = QStringLiteral("刚性中心间隔棒_%1").arg(mpcId);
            mpc->m_pMasterNode = master;
            mpc->m_pSlaveNode = slave;
            mpc->m_Offset = Vector3d(slave->m_X, slave->m_Y, slave->m_Z) - center;
            mpc->m_SlaveDirections = {0, 1, 2};
            if (!m_structure->m_MPCConstraints.emplace(mpcId, mpc).second)
            {
            ...
            auto twistMpc = std::make_shared<AxialTwistTieMPCConstraint>();
            twistMpc->m_Id = twistMpcId;
            twistMpc->m_Name = QStringLiteral("刚性中心间隔棒轴向扭转_%1").arg(twistMpcId);
            twistMpc->m_pMasterNode = master;
            twistMpc->m_pSlaveNode = slave;
            twistMpc->m_Axis = wireAxes[wireId];
            twistMpc->m_SlaveDirections = {3};
            if (!m_structure->m_MPCConstraints.emplace(twistMpcId, twistMpc).second)
            {
                error = "添加刚性中心间隔棒轴向扭转 MPC 失败";
                rollback();
                return false;
            }
            createdMpcIds.push_back(twistMpcId);
            spacer.mpcIds.push_back(twistMpcId);
        }
        if (!AddRigidSpacerInertia(centerNodeId, wirePositions, config.property, spacer.rigidBodyInertiaId, error))
        {
            rollback();
            return false;
        }
...
bool ConductorModelBuilder::BuildInnerSpacers(_OUT LineBuildResult& line, const std::vector<InnerSpacerConfig>& configs,
                                              _OUT std::string& error)
{
...
bool ConductorModelBuilder::CalculateInnerSpacerConfigs(const LineBuildResult& line,
                                                        ...
                                                        _OUT std::string& error) const
{
    ...
    {
        const std::vector<double> positions = CalculateStandardInnerSpacerPositions(line.spanLength);
...
bool ConductorModelBuilder::AddNodes(BundleResult& raw, const LineBuildConfig& config, _OUT LineBuildResult& result,
                                     _OUT std::string& error)
{
    std::vector<int> createdNodeIds;
    auto rollback = [&]()
    ...
    const bool convergeEnds = config.convergeBundleEnds && config.conductor.nBundle > 1;
    const bool directWireSupports = convergeEnds && config.endTopology == BundleEndTopology::DirectWireSupports;
    ...
    if (convergeEnds && !directWireSupports)
    {
        ...
        if (!dualSupport &&
            (!addNode(config.start, result.leftSupportNodeId) || !addNode(config.end, result.rightSupportNodeId)))
        {
            error = "添加耐张导线挂点失败";
            rollback();
            return false;
        ...
        for (const auto& pair : raw.wiresNode)
        {
            ...
            leftSums[group] += Vector3d((nodes[0].x + nodes[1].x) * 0.5, (nodes[0].y + nodes[1].y) * 0.5,
                                        (nodes[0].z + nodes[1].z) * 0.5);
            const std::size_t last = nodes.size() - 1;
            rightSums[group] +=
                Vector3d((nodes[last - 1].x + nodes[last].x) * 0.5, (nodes[last - 1].y + nodes[last].y) * 0.5,
        ...
        if (dualSupport)
        {
            ...
            const double spacing =
                config.dualSupportSpacing > 1.0e-7 ? config.dualSupportSpacing : config.conductor.spacing;
...
bool ConductorModelBuilder::AddElements(const BundleResult& raw, const LineBuildConfig& config,
                                        std::shared_ptr<Property> property, _OUT LineBuildResult& result,
                                        _OUT std::string& error)
{
    std::vector<int> createdElementIds;
    auto rollback = [&]()
...
bool ConductorModelBuilder::AddTensionEndElements(const LineBuildConfig& config, _OUT LineBuildResult& result,
                                                  _OUT std::string& error)
{
    ...
    auto buildSide = [&](TensionEndModel& endModel) -> bool
    {
        for (std::size_t groupIndex = 0; groupIndex < endModel.groupNodeIds.size(); ++groupIndex)
        {
            ...
            const int supportNodeId = endModel.supportNodeIds.size() == endModel.groupNodeIds.size()
                                          ? endModel.supportNodeIds[groupIndex]
                                          : endModel.supportNodeId;
            int elementId = -1;
            if (!AddElement(supportNodeId, groupNodeId, config.elementType, config.property,
                            config.conductor.initialStress, elementId, error, ElementRole::TensionHardware))
                return false;
            createdElementIds.push_back(elementId);
        ...
        {
            int elementId = -1;
            if (!AddElement(endModel.groupNodeIds[0], endModel.groupNodeIds[1], config.endFittingElementType,
                            stabilizerProperty, config.conductor.initialStress, elementId, error,
...
bool ConductorModelBuilder::CreateSubConductorSets(const LineBuildConfig& config, _OUT LineBuildResult& result,
                                                   _OUT std::string& error)
{
    ...
    for (auto& pair : result.subConductors)
    {
        ...
        const QString wireName = QStringLiteral("%1-分裂线%2").arg(prefix).arg(sub.wireId + 1);
        ...
        sub.nodeSetId =
            m_structure->AddModelSet(wireName + QStringLiteral("-节点集"), ModelSetType::Node, nodeIds, &setError);
        if (sub.nodeSetId <= 0)
        {
        ...
        sub.elementSetId = m_structure->AddModelSet(wireName + QStringLiteral("-气动单元集"), ModelSetType::Element,
                                                    elementIds, &setError);
...
bool ConductorModelBuilder::RenumberLineModel(LineBuildResult& result, _OUT std::string& error)
{
    ...
    generatedNodeIds.insert(result.leftTensionEnd.groupNodeIds.begin(), result.leftTensionEnd.groupNodeIds.end());
    generatedNodeIds.insert(result.rightTensionEnd.groupNodeIds.begin(), result.rightTensionEnd.groupNodeIds.end());
    generatedElementIds.insert(result.leftTensionEnd.yokeElementIds.begin(),
                               result.leftTensionEnd.yokeElementIds.end());
    generatedElementIds.insert(result.rightTensionEnd.yokeElementIds.begin(),
                               result.rightTensionEnd.yokeElementIds.end());
    if (result.leftTensionEnd.stabilizerElementId > 0)
    ...
    for (const auto& suspension : result.suspensionPoints)
    {
        ...
        generatedNodeIds.insert(suspension.wireNodeIds.begin(), suspension.wireNodeIds.end());
        if (suspension.spacerCenterNodeId > 0)
            generatedNodeIds.insert(suspension.spacerCenterNodeId);
        generatedNodeIds.insert(suspension.spacerInnerNodeIds.begin(), suspension.spacerInnerNodeIds.end());
        generatedElementIds.insert(suspension.yokeElementIds.begin(), suspension.yokeElementIds.end());
        if (suspension.stringElementId > 0)
            generatedElementIds.insert(suspension.stringElementId);
        generatedElementIds.insert(suspension.spacerElementIds.begin(), suspension.spacerElementIds.end());
    }
    for (const auto& spacer : result.innerSpacers)
    {
    ...
    const std::size_t groupCount =
        std::min(result.leftTensionEnd.groupNodeIds.size(), result.rightTensionEnd.groupNodeIds.size());
    if (groupCount > 0)
    {
    ...
    if (nodeOrder.size() != generatedNodeIds.size() || elementOrder.size() != generatedElementIds.size())
    {
