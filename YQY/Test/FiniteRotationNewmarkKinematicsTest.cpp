...
namespace
{
void CheckVector(const Eigen::Vector3d& actual, const Eigen::Vector3d& expected, double tolerance, const char* message)
{
    if ((actual - expected).norm() > tolerance)
        throw std::runtime_error(message);
}

void CheckMatrix(const Eigen::Matrix3d& actual, const Eigen::Matrix3d& expected, double tolerance, const char* message)
{
    if ((actual - expected).norm() > tolerance)
        throw std::runtime_error(message);
}
...
int main()
{
    ...
    CheckVector(Eigen::Vector3d(trussNode.m_Displacement[0], trussNode.m_Displacement[1], trussNode.m_Displacement[2]),
                dt * Eigen::Vector3d(1.0, -2.0, 0.5) + dt * dt * (0.5 - beta) * Eigen::Vector3d(0.2, 0.4, -0.6),
                1.0e-12, "3 DOF predictor");
    trussNode.ApplyNewmarkCorrection(Eigen::Vector3d(0.01, 0.02, -0.03), Eigen::Vector3d::Zero(), a0, a1);
    trussNode.RollbackNewmarkStep();
    CheckVector(Eigen::Vector3d(trussNode.m_Velocity[0], trussNode.m_Velocity[1], trussNode.m_Velocity[2]),
                Eigen::Vector3d(1.0, -2.0, 0.5), 1.0e-12, "3 DOF rollback");
    ...
    const double twistPredictor = 0.1 + dt * 0.3 + dt * dt * (0.5 - beta) * -0.2;
    ...
    cableNode.ApplyNewmarkCorrection(Eigen::Vector3d::Zero(), Eigen::Vector3d(deltaTwist, 0.0, 0.0), a0, a1);
    if (std::abs(cableNode.m_Velocity[3] - (0.3 + dt * (1.0 - gamma) * -0.2 + a1 * deltaTwist)) > 1.0e-12)
    ...
    if (std::abs(cableNode.m_Displacement[3] - 0.1) > 1.0e-12 || std::abs(cableNode.m_Velocity[3] - 0.3) > 1.0e-12 ||
    ...
    const Eigen::Vector3d Hpred = dt * omegaMaterial_n + dt * dt * (0.5 - beta) * alphaMaterial_n;
    ...
    CheckVector(node.m_OmegaMaterial, omegaMaterial_n + dt * (1.0 - gamma) * alphaMaterial_n, 1.0e-12,
                "material angular velocity predictor");
    CheckVector(node.m_AlphaMaterial, Eigen::Vector3d::Zero(), 1.0e-12, "material angular acceleration predictor");
    ...
    Utility::CR::Extract_RotationVector(correctedRotation * initialRotation.transpose(), correctedStepRotation);
    const Eigen::Vector3d deltaH = initialRotation.transpose() * (correctedStepRotation - oldStepRotation);
    ...
    node.ApplyNewmarkCorrection(Eigen::Vector3d(0.001, -0.002, 0.003), deltaRotation, a0, a1);
    CheckMatrix(node.m_Rg, correctedRotation, 1.0e-12, "multiplicative correction");
    CheckVector(node.m_OmegaMaterial, omegaExpected, 1.0e-12, "material angular velocity correction");
    CheckVector(node.m_AlphaMaterial, alphaExpected, 1.0e-12, "material angular acceleration correction");
    CheckMatrix(node.m_Rg.transpose() * node.m_Rg, Eigen::Matrix3d::Identity(), 1.0e-12, "rotation orthogonality");

    node.RollbackNewmarkStep();
    CheckMatrix(node.m_Rg, initialRotation, 1.0e-12, "rotation rollback");
    CheckVector(Eigen::Vector3d(node.m_Velocity[3], node.m_Velocity[4], node.m_Velocity[5]), omegaSpatial_n, 1.0e-12,
                "angular velocity rollback");
    CheckVector(Eigen::Vector3d(node.m_Acceleration[3], node.m_Acceleration[4], node.m_Acceleration[5]), alphaSpatial_n,
                1.0e-12, "angular acceleration rollback");
